...
import { APIPromise } from 'isaacus/core/api-promise';

import util from 'node:util';
import Isaacus from 'isaacus';
import { APIUserAbortError } from 'isaacus';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['ISAACUS_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Isaacus) => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Isaacus({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Isaacus({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Isaacus({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['ISAACUS_LOG'] = 'not a log level';
      const client = new Isaacus({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'ISAACUS_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['ISAACUS_LOG'] = 'debug';
      const client = new Isaacus({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  test('normalized method', async () => {
    ...
    const client = new Isaacus({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Isaacus({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['ISAACUS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Isaacus({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Isaacus({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['ISAACUS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Isaacus({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Isaacus({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Isaacus({ apiKey: 'My API Key' });
  ...
  describe('withOptions', () => {
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Isaacus({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['ISAACUS_API_KEY'] = 'My API Key';
    const client = new Isaacus();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['ISAACUS_API_KEY'] = 'another My API Key';
    const client = new Isaacus({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Isaacus({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Isaacus({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Isaacus({ apiKey: 'My API Key', fetch: testFetch });
